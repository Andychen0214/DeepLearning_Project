...
import numpy as np
import time 
#用六層CNN的架構來訓練CIFAR-10資料集，只要解除註解就是Q2-2-4的答案，並且把Q2-2-3的圖畫出來
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
BATCH_SIZE = 128
EPOCHS = 150 

MODEL_SAVE_PATH = 'cifar_cnn_6layer_gap_NO_L2.pth' 
...
print("Using SGD Optimizer (NO Weight Decay)...")
# 當有weight_decay時就是L2正規化
optimizer = optim.SGD(
    model.parameters(), 
    lr=0.1,            
    momentum=0.9,       
...
plt.title('Learning Curve (6-Layer + GAP + NO L2)')
...
plt.title('Accuracy Curve (6-Layer + GAP + NO L2)')
plt.xlabel('Iteration')
plt.ylabel('Accuracy (%)')
plt.legend()
plt.grid(True)
...
plt.tight_layout()
plt.savefig('results_curve_cifar_6layer_gap_NO_L2.png')
plt.show()

plt.figure(figsize=(15, 12))

def plot_histogram(layer_name, layer_weights, layer_biases, subplot_idx):
    plt.subplot(5, 2, subplot_idx * 2 - 1) # 5 行 2 列
    plt.hist(layer_weights.flatten(), bins=100)
    plt.title(f'Histogram of {layer_name} Weights (NO L2)')
    plt.xlabel('Value')
    plt.ylabel('Number')
    
    plt.subplot(5, 2, subplot_idx * 2)
    plt.hist(layer_biases.flatten(), bins=100)
    plt.title(f'Histogram of {layer_name} Biases (NO L2)')
    plt.xlabel('Value')
    plt.ylabel('Number')
...
plt.tight_layout()
plt.savefig('histograms_cifar_6layer_gap_NO_L2.png')
plt.show()
