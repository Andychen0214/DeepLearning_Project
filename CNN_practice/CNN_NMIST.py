...
import numpy as np
#只要加入Weight Decay的部分就是Q1-1-4的內容
...
class myCNN(nn.Module):
    def __init__(self):
        ...
        # 建立我的卷積層
        # 用法:nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding)
        # MNIST 是 1 個灰階通道的圖片，大小是 28x28
        # (W - F + 2P) / S + 1 = (28 - 5 + 2*2) / 1 + 1 = 28. -> Output: [batch_size, 16, 28, 28]
        #padding=2 是為了保持輸出大小不變 用上面的公式可以算出來需要多少才可以保持出來的大小不變
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=16, kernel_size=5, stride=1, padding=2)
        self.relu1 = nn.ReLU() #激活
        # (28 - 2) / 2 + 1 = 14. -> Output: [batch_size, 16, 14, 14] 16張14*14的特徵圖的意思
        ...
        # 全連接層 (Dense Layer)
        # 32 (channels) * 7 (height) * 7 (width) = 1568 阿後面那個64是隱藏層的神經元數量
    ...
    def forward(self, x):
        # 網路架構 先卷積1->激活->池化->卷積2->激活->池化->攤平->全連接->激活->全連接
...
#這邊加入weight_decay就是L2正規化, Q2-1-4的內容)
...
plt.tight_layout()
results_plot_path = 'results_curve.png'
...
plt.tight_layout()
histogram_plot_path = 'histograms.png'
